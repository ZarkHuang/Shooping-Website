<div id="card--box" class="bg-white pl-3 pt-2 pr-3" style="width: 300px;height: calc(100vh - 200px);position: absolute;margin-left: calc(100% - 323px);margin-top: 70px;border-radius: 10px;z-index: 999;display: none;">
    ...
    <a href="/shoppingWebsite/payment/checkout">
...
<script type="text/javascript">
    ...
    function addToCart(id,name,image,price){
        var cart =JSON.parse(localStorage.getItem('cart'));
        console.log(id,name,image,price); //序列化成 JSON 字串
        var checkCart=0;
        var quanlity = 1;
        // 取得物件的指定值
        if(cart.length > 0){
    ...
    function cartPrinf(){
     $( document ).ready(function() {
        ...
        for(let i = 0; i < cart.length; i++) {
            ...
            var nameT = cart[i].name;
            var contentT = cart[i].content;
            ...
            <img src="/shoppingWebsite/public/images/product/`+imageLinkcut+`" width="100%" height="100%" style="object-fit: contain">
            </div>
            <div class="pl-1" style="width: calc(100% - 80px);height: 80px;">
            <p class="font-weight-bold mb-0"> `+nameT+` </p>
            <p class="mb-1" style="font-size: 75%;margin-top: -2px"> Description </p>
